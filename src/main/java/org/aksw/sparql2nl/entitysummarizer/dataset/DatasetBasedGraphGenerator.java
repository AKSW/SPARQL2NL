...
public class DatasetBasedGraphGenerator {
	...
	private Set<String> blacklist = Sets.newHashSet("http://dbpedia.org/ontology/wikiPageExternalLink");
...
//		qef = new QueryExecutionFactoryDelay(qef, 500);
	...
	public Map<NamedClass, WeightedGraph> generateGraphs(double threshold, String namespace){
		Map<NamedClass, WeightedGraph> graphs = new HashMap<>();
	...
	public WeightedGraph generateGraph(NamedClass cls, double threshold, String namespace){
		...
		Map<Set<ObjectProperty>, Integer> cooccurrences = getCooccurrences(cls, properties);
		
		//create the weighted graph
		WeightedGraph wg = new WeightedGraph();
		LoadingCache<ObjectProperty, Node> property2Node = CacheBuilder.newBuilder()
			       .maximumSize(properties.size())
			       .build(
		...
		for (Entry<Set<ObjectProperty>, Integer> entry : cooccurrences.entrySet()) {
	...
	private Map<Set<ObjectProperty>, Integer> getCooccurrences(NamedClass cls, Set<ObjectProperty> properties){
		Map<Set<ObjectProperty>, Integer> pair2Frequency = new HashMap<Set<ObjectProperty>, Integer>();
	...
	private ResultSet executeSelectQuery(String query){System.out.println(query);
	...
	public static void main(String[] args) throws Exception {
		...
		new DatasetBasedGraphGenerator(endpoint, "cache").
		generateGraph(new NamedClass("http://dbpedia.org/ontology/AmericanFootballPlayer"), 0.5, "http://dbpedia.org/ontology/");
